...
export const getImg = async(name) => {
  ...
  try {
    const axiosRes = await axios.get(`http://localhost:3000/getImg/`+ name);    //TODO move server address to env file
...
export const getImgsGallery = async(name) => {
  let result = [DEFAULT_IMG];
  if(true){
    const axiosRes = await axios.get(`http://localhost:3000/getImgsGallery/`+ name);  //TODO move server address to env file
    result = axiosRes.data;
  }
  return result;
}
